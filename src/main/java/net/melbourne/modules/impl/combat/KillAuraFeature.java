...
public class KillAuraFeature extends Feature {
    ...
    public final ModeSetting limbMode = new ModeSetting("Limb", "Where to aim", "Auto", new String[]{"Auto", "Head", "Chest", "Feet"});
    public final ModeSetting swapMode = new ModeSetting("SwapMode", "Sword handling", "Switch", new String[]{"None", "Switch", "Require"});

    public final WhitelistSetting targets = new WhitelistSetting("Targets", "Entities to attack", WhitelistSetting.Type.CUSTOM,
            new String[]{"Players"}, new String[]{"Players", "Monsters", "Animals"});
    ...
    public final BooleanSetting rotate = new BooleanSetting("Rotate", "Rotate to attack target", true);
    ...
    public void onTick(TickEvent e) {
        ...
        if (rotate.getValue()) {
            ...
            float[] rotations = RotationUtils.getRotationsTo(mc.player.getEyePos(), aimPos);
            Services.ROTATION.setRotationPoint(new RotationPoint(rotations[0], rotations[1], 200, true));
        }
    ...
    public void onRenderWorld(RenderWorldEvent e) {
        if (target == null || getNull()) return;
        long now = System.currentTimeMillis();
        long elapsed = now - hitTime;
        if (elapsed >= hitFadeMillis) return;
    ...
    private boolean handleSwordSwap() {
        ...
        if (swordSlot == -1) return mode.equals("Switch");

        if (mode.equals("Require") && mc.player.getInventory().getSelectedSlot() != swordSlot) return false;
        if (mode.equals("Switch") && mc.player.getInventory().getSelectedSlot() != swordSlot) {
    ...
    private boolean isNotFriend(Entity e) {
        if (!(e instanceof PlayerEntity player)) return true;
    ...
    private boolean isValidTarget(Entity entity) {
        if (entity instanceof PlayerEntity) return targets.getWhitelistIds().contains("Players");
        if (entity instanceof HostileEntity) return targets.getWhitelistIds().contains("Monsters");
        if (entity instanceof AnimalEntity) return targets.getWhitelistIds().contains("Animals");
        return false;
    }

    private Vec3d getAimPosition(Entity entity) {
        double yOffset = switch (limbMode.getValue()) {
            case "Head" -> entity.getHeight() * 0.9;
            case "Chest" -> entity.getHeight() * 0.65;
            case "Feet" -> entity.getHeight() * 0.1;
            default -> entity.getHeight() * 0.75;
        };

        if (limbMode.getValue().equals("Auto")) {
            Vec3d bestHit = null;
            double bestDistanceSq = Double.MAX_VALUE;

            double[] offsets = {entity.getHeight() * 0.9, entity.getHeight() * 0.65, entity.getHeight() * 0.1};
            for (double offset : offsets) {
                Vec3d checkPos = entity.getPos().add(0, offset, 0);
                if (canSee(checkPos)) {
                    double distSq = mc.player.getEyePos().squaredDistanceTo(checkPos);
        ...
        return entity.getPos().add(0, yOffset, 0);
    }

    private boolean canSee(Entity e) {
        return canSee(e.getLerpedPos(1f).add(0, e.getHeight() * 0.5, 0));
    }

    private boolean canSee(Vec3d target) {
        Vec3d eye = mc.player.getCameraPosVec(1f);
        HitResult hit = mc.world.raycast(new RaycastContext(
                eye, target, RaycastContext.ShapeType.COLLIDER, RaycastContext.FluidHandling.NONE, mc.player));

        return hit.getType() == HitResult.Type.MISS || mc.player.getEyePos().squaredDistanceTo(target) < mc.player.getEyePos().squaredDistanceTo(hit.getPos());
    }

    @Override public String getInfo() { return targetMode.getValue(); }

    public boolean getNull() { return mc.player == null || mc.world == null || mc.interactionManager == null; }
}